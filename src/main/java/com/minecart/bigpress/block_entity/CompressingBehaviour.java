...
public class CompressingBehaviour extends BeltProcessingBehaviour {
        ...
        switch (phase) {
            case EXTEND:
                if (runningTicks >= BREAKTIME / 2) {
                    ...
                    if (level.getBlockState(worldPosition.below(1))
    ...
    public void write(CompoundTag nbt, HolderLookup.Provider registries, boolean clientPacket) {
        ...
        nbt.putInt("Mode", mode.ordinal()); // Save Mode
        ...
        if (clientPacket) {
            nbt.put("ParticleItems", NBTHelper.writeCompoundList(particleItems, s -> (CompoundTag) s.saveOptional(registries)));
            particleItems.clear();
        }
    ...
    public void read(CompoundTag nbt, HolderLookup.Provider registries, boolean clientPacket) {
        ...
        mode = Mode.values()[nbt.getInt("Mode")]; // Load Mode
